...
function loadCharacter() {
    ...
    // Clan toujours affiché, 'aucun' si absent
    ...
    if (char.clan) {
        ...
        document.getElementById('clanField').textContent = 'Aucun';
    }
    clanBlock.style.display = 'block';

    document.getElementById('bioText').textContent = char.bio;
    
    // Gestion du portrait avec fallback
    ...
    if (char.portrait) {
        ...
        testImg.onerror = function() {
            ...
            const placeholderUrl = `https://via.placeholder.com/400x500/1a1f2e/c8aa6e?text=${encodeURIComponent(char.name)}`;
            portraitImg.src = placeholderUrl;
        };
        testImg.src = char.portrait;
    } else {
        // Pas d'image spécifiée, utiliser placeholder
        const placeholderUrl = `https://via.placeholder.com/400x500/1a1f2e/c8aa6e?text=${encodeURIComponent(char.name)}`;
        portraitImg.src = placeholderUrl;
    }
