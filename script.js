...
function initializeCharacters() {
    ...
    charactersData.forEach(charData => {
        const character = {
            ...
            grade: charData.title, // Utiliser le titre comme grade
            description: charData.description,
            image: '',
            village: charData.village,
            clan: charData.clan,
            dataName: charData.dataName,
            customTitle: charData.customTitle || null
        };
...
function loadCharacterImage(characterName, dataName, imgElement, character) {
    ...
    // Priorité aux formats webp puis png, jpg, jpeg
...
function showAlphabeticalMode() {
    ...
    // Mettre à jour l'affichage des personnages selon le village actuel
    updateCharactersDisplay();
}
...
function createAlphabeticalCard(character) {
    ...
    const displayTitle = character.customTitle || character.title;
    ...
    card.innerHTML = `
        ...
        <div class="alphabetical-character-info">
            ...
            <p class="character-title">${displayTitle}</p>
        </div>
    `;
...
// Définition des hiérarchies de grades par village (ordre inversé: plus prestigieux en haut)
const gradeHierarchies = {
    konoha: [
        'Hokage',
        'Conseiller du Hokage',
        'Sanin Légendaire',
        'Commandant Jonin',
        'Jonin',
        'Tokubetsu Jonin',
...
function updateCharactersDisplay() {
    ...
    // Utiliser le tri alphabétique pour 'tous les shinobis', tri par grade pour les villages spécifiques
    if (currentVillage === 'all') {
        showAlphabeticalGrid(filteredCharacters);
    } else {
        showGradeBasedGrid(filteredCharacters);
    }
    
    console.log('👥 Personnages affichés:', filteredCharacters.length);
}

// Afficher la grille alphabétique (pour 'tous les shinobis')
function showAlphabeticalGrid(characters) {
    const alphabeticalGrid = document.getElementById('alphabeticalGrid');
    const gradeBasedGrid = document.getElementById('gradeBasedGrid');
    
    if (alphabeticalGrid) {
        alphabeticalGrid.style.display = 'flex';
        alphabeticalGrid.innerHTML = '';
    ...
    if (gradeBasedGrid) {
        gradeBasedGrid.style.display = 'none';
    }
}

// Afficher la grille par grades (pour les villages spécifiques)
function showGradeBasedGrid(characters) {
    const alphabeticalGrid = document.getElementById('alphabeticalGrid');
    const gradeBasedGrid = document.getElementById('gradeBasedGrid');
    ...
    if (gradeBasedGrid) {
        gradeBasedGrid.style.display = 'flex';
        gradeBasedGrid.innerHTML = '';
        
        // Grouper les personnages par grade (pas besoin de section village car on est déjà filtré)
        const charactersByGrade = {};
        
        characters.forEach(character => {
            if (!charactersByGrade[character.grade]) {
                charactersByGrade[character.grade] = [];
            }
            charactersByGrade[character.grade].push(character);
        });
        
        // Obtenir la hiérarchie de grades pour le village actuel
        const gradeHierarchy = gradeHierarchies[currentVillage];
        
        if (gradeHierarchy) {
            // Créer des sections pour chaque grade dans l'ordre de la hiérarchie
            gradeHierarchy.forEach(grade => {
                // Afficher toujours la section, même si aucun personnage
                const gradeSection = document.createElement('div');
                gradeSection.className = 'grade-section';
                
                const gradeTitleContainer = document.createElement('div');
                gradeTitleContainer.className = 'grade-title-container';
                ...
                const gradeArrows = document.createElement('div');
                gradeArrows.className = 'grade-arrows';
                gradeArrows.innerHTML = '<div class="grade-line"></div><div class="grade-line"></div>';
                gradeTitleContainer.appendChild(gradeArrows);
                
                gradeSection.appendChild(gradeTitleContainer);
                
                const gradeContent = document.createElement('div');
                gradeContent.className = 'grade-section-content';
                
                // Ajouter les personnages s'il y en a
                if (charactersByGrade[grade] && charactersByGrade[grade].length > 0) {
                    
                    // Trier les personnages du même grade par nom
                    const sortedGradeCharacters = [...charactersByGrade[grade]].sort((a, b) => 
                        a.displayName.localeCompare(b.displayName, 'fr')
                    );
                    
                    sortedGradeCharacters.forEach(character => {
                        const card = createGradeBasedCard(character);
                        gradeContent.appendChild(card);
                    });
                }
                
                gradeSection.appendChild(gradeContent);
                gradeBasedGrid.appendChild(gradeSection);
            });
        }
    }
    
    console.log('📋 Grille par grades créée avec', characters.length, 'personnages');
}

// Obtenir le nom d'affichage du village
function getVillageDisplayName(village) {
    ...
    return villageNames[village] || village.toUpperCase();
}

// Création d'une carte basée sur les grades
function createGradeBasedCard(character) {
    const card = document.createElement('div');
    card.className = 'alphabetical-character-card';
    card.setAttribute('data-name', character.dataName);
    card.setAttribute('data-village', character.village);
